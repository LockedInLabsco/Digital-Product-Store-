...
export default function ProductCard({
  ...
  return (
    ...
    >
      <article className="flex h-full flex-col overflow-hidden rounded-md border border-line bg-ink transition-colors duration-200 group-hover:border-neutral-400">
        <ProductPreview
          previews={previews}
          productTitle={title}
        ...
        <div className="flex flex-1 flex-col p-6">
          ...
          <h3 className="mt-2 font-sans font-bold tracking-tight text-xl leading-snug text-cream">{title}</h3>
          <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-beige">
            {description}
          </p>

          <div className="mt-5 flex items-center justify-between border-t border-line pt-4">
            <span className="font-sans font-bold tracking-tight text-lg text-cream">{formatPrice(price)}</span>
            <span className="flex items-center gap-1 text-sm font-medium text-beige">
