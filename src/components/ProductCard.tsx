...
export default function ProductCard({
  ...
  return (
    <Link
      href={`/products/${slug}`}
      onClick={handleClick}
      className="group perspective-1000 block h-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-4 focus-visible:ring-offset-ink"
      data-reveal="up"
      data-reveal-delay={String(Math.min(revealDelay, 2))}
    >
      <DepthCard maxTilt={4} className="h-full">
        <article className="flex h-full flex-col overflow-hidden rounded-sm border border-line/10 bg-offwhite transition-all duration-300 group-hover:border-gold/30 group-hover:shadow-[0_24px_60px_-24px_rgba(0,0,0,0.7)]">
          <ProductPreview
            previews={previews}
            productTitle={title}
